...
1. 确保手机和电脑连接同一 WiFi
2. 手机浏览器访问 `http://<电脑IP>:5000/mobile`
3. 点击"开始采集"授权传感器权限（iOS需用户手势确认）
4. 手机端页面实时显示传感器读数，数据自动发送到电脑端
5. 电脑端仪表盘自动切换为"📱 手机实时数据"模式
6. 手机保持屏幕常亮以确保数据连续采集

**注意**：手机端需要支持 DeviceMotion API（iOS 12.2+ 或 Android 4.4+），GPS 数据需要 HTTPS 或 localhost 环境。
